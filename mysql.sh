...
mysql -h mysql.erothi.online -u root -pExpenseApp@1 -e "show databases;" >> $LOG_FILE_NAME # Test the MySQL connection by showing databases and log the output

if [ $? -ne 0 ]
then
    echo "MySQL Root Password Not Set Properly" >> $LOG_FILE_NAME # Log the error message if the connection test fails
    mysql_secure_installation --set-root-pass=ExpenseApp@1 >> $LOG_FILE_NAME # Set the root password using mysql_secure_installation and log the output
